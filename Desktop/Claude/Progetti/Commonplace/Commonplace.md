...
**Stato attuale:** âœ… Completo con Supabase. **2026-07-20 (#28):** corretto il bottone «Completa scheda», che era cieco su 5 sezioni su 11 (`cardNeedsRepair` e `repairCard` avevano due liste divergenti — ora una sola tabella `CARD_SECTIONS`), e aggiunto il **menu «↺ Rigenera ▾»** con scelta a spunte delle sezioni, che a differenza della riparazione sostituisce anche ciò che è già completo. SW v29 — ⚠️ **committato ma NON deployato**.
...
**Stato attuale:** Funzionante — revisione completa 2026-06-09 (Sessione #18). ⚠️ Da fare: impostare nuova `ANTHROPIC_API_KEY` su Vercel (progetto Syllabus) + redeploy. **2026-07-20 (#28):** aggiunte le **risorse «già note»** (step 4 del wizard: escluse dal percorso E date per acquisite, così l'AI riparte da lì invece di accorciare) e la **progressione a tappe motivate** (chiamata AI separata dopo la selezione delle risorse; la call A non è stata toccata; i percorsi esistenti hanno «↓ Genera la progressione» che li ordina senza rigenerarli). I campi nuovi vivono in `raw_data`, **nessuna migration**. SW v6 — ⚠️ **committato ma NON deployato**. 🔍 Vedi i tre bug latenti in coda al diario (#28): `sl_resources` è morta (0 righe, dati in `raw_data`), `updateCurriculum` riceve non-colonne, e `supabase-migration.sql` **non descrive più lo schema reale** — non usarlo per ricreare il database.
...
⚠️ **Azioni richieste a Stefano:**
- **Deploy da autorizzare**: Footnote (SW v29) e Syllabus (SW v6). Nessuno dei due è live.
  Al deploy va alzato anche `SW_MIN` in `Suite/collauda.cjs` (footnote 28→29, syllabus 5→6):
  **non l'ho alzato ora** perché renderebbe rosso il collaudo finché non si deploya.
- **Push non fatto** (mai senza richiesta esplicita). Ricorda: **mai push da `Syllabus/app`**,
