...
> Ultimo aggiornamento: 2026-07-24 -- Sessione #30 (Opus 4.8). **DnD Master — «🏛 Aggiorna reputazione del giocatore».** L'alias/nascondi del prestigio raggiungeva il giocatore solo al seed (all'assegnazione): impostarlo DOPO non arrivava mai (in v1 niente sync amministrativa master→giocatore). Root cause verificata sui dati reali (la riga condivisa aveva ancora «Clero», nome vero). Fix mirato — NON un re-seed (che cancellerebbe la scheda del giocatore): `refreshPublicPrestige` (puro, preserva i valori del giocatore per id) + `refreshSharedPrestige` (riscrive solo `char.prestige`, RLS `dsc_update_own` master verificata) + bottone nella vista Master. 238 test verdi, deploy Netlify verificato. **+ BookShelf — statistiche per anno/mese vuote:** `read_year`/`read_month` sono colonne text → i confronti numerici (`=== scopeYear`) erano sempre falsi; coercizione a `Number` al load, SW v3, deploy Vercel verificato. Entrambi verificati live via curl; **prove visive finali a Stefano** (app dietro login; per DnD serve il flusso a due account). — Precedente #29 (Opus 4.8). **🍷 Cru — nuova app cantina portata a maturità e resa condivisibile con amici.** Dalla nascita (single-file pattern Marginalia) a: inventario a movimenti, CRUD, scheda AIS, finestra di bevuta stimata+filtro a chip, voto Vivino 1–5, gradazione, **scan barcode con fotocamera** (BarcodeDetector + lookup a cascata OFF→Open Products Facts→UPCitemdb, copertura vini comunque parziale), dataset denominazioni **91→251** (Italia+estero), icone PWA PNG (prima solo SVG → Android non installava la PWA), voto al "bevuto", **archivio** vini finiti, **posizione in cantina**, **wishlist** (con "sposta in cantina"), **export CSV/JSON**. **Multi-utente**: Cru MIGRATA su un progetto Supabase **DEDICATO** (`fqdccttzuaopoaojshvw`) su un **SECONDO account** di Stefano (isolata dalla suite personale — il piano Free è 2 progetti/account e il primo era pieno), con registrazione in-app e confirm-email off; RLS `user_id=auth.uid()` → ognuno vede solo la sua cantina. **⚠️ ROOT CAUSE (ore di deploy persi):** i deploy `vercel` da una cartella-repo git allegano l'email autore del commit HEAD, e **Vercel BLOCCA il deploy se quell'email non è di un account GitHub** — appare come deploy infiniti `UNKNOWN`/"Building…", causa leggibile SOLO nella dashboard Inspect, non dalla CLI; il workspace usa il placeholder `tua@email.com` → fix: `git config user.email olengard@gmail.com` nel repo Cru + `git commit --amend --reset-author`. **Git**: Cru ha un repo **dedicato e privato** `github.com/Olengard/cru` (NON syllabus, per azzerare il rischio force-push; unica single-file in git). SW `cru-v9`, live su `cru.commonplaceapp.org`. **Dettaglio completo in `Cru/CLAUDE.md` (nuovo).** ⚠️ **Spetta a Stefano**: ricaricare i suoi vini sul nuovo progetto (i 3 di test erano su pchld, non migrati), cancellare l'utente di collaudo `cru-selftest-…@example.com` dal dashboard. Proposto ma NON costruito: "Diario di bevuta" (timeline delle occasioni). — Precedente #28d (Opus 4.8). **Stefano: "ho escluso McCarthy e i primi due libri sono suoi — sbaglio io?" NON sbagliava lui.** Verificato sui dati: tutti i suoi percorsi hanno `avoidList` VUOTA, quindi il filtro deterministico (#28c, corretto) non aveva nulla da filtrare. Due cause di UI, entrambe mie: (1) il campo a chip **buttava via il testo non confermato** — scrivere "McCarthy" e cliccare Continua senza premere Invio/＋ lo perdeva (è lo stesso bug che in #28c avevo liquidato come "artefatto del test": era vero); ora `onBlur` committa il pendente. (2) Stefano scrive l'esclusione **nell'argomento** ("Oltre McCarthy…"), non nel campo, due volte su due; aggiunto il rilevamento che la propone come chip da confermare (opt-in). Difesa ora a tre strati: suggerimento dall'argomento + commit del campo + filtro deterministico. Deployato SW v10, collaudo verde. ⚠️ Lezione: gli eventi sintetici non attraversano la delega focusout di React — per validare focus/blur serve il mouse reale. — Precedente #28c (Opus 4.8). **Due segnalazioni di Stefano su Syllabus dal vivo (SW v9).** (1) Le esclusioni non tenevano: McCarthy rientrava malgrado il divieto. Il prompt da solo non basta — il modello non obbedisce in modo affidabile. Aggiunto uno **scarto deterministico** (`filterExcluded`): dopo la generazione confronta titolo+autore con la lista e rimuove i corrispondenti, poi il top-up rimpiazza. Non dipende dall'obbedienza del modello, quindi i test simulati sono prova piena (6 casi unitari + integrazione, 0 McCarthy nel risultato). Filtra solo «da evitare», non «già note», e non tocca le descrizioni (il commento su Bloom resta). (2) La **PWA non si installava con la sua icona**: il manifest puntava tutto a un favicon da 646 byte, e le icon-192/512 presenti erano quelle di BookShelf. Generate le icone dedicate di Syllabus (la "S" oro Playfair, gemella delle altre app) su canvas del browser e ricucito il manifest. ⚠️ Stefano: reinstallare la PWA (disinstallare prima, l'icona vecchia resta in cache OS). — Precedente #28b (Opus 4.8). **Segnalazione di Stefano: "l'exclude di Syllabus non funziona" — il filtro NON era rotto, non era mai entrato in funzione.** Verificato sui dati: il campo era vuoto, l'esclusione era stata scritta nell'ARGOMENTO in prosa ("Oltre McCarthy…") e l'AI l'aveva letta come un tema SU McCarthy, includendo Blood Meridian come "opera-fonte da rileggere criticamente" — e la progressione nuova gliel'aveva messa in TAPPA 0, cioè in apertura. **Root cause: errore di progettazione, non di codice** — lo step 4 non permetteva di dire "non lo voglio", solo "non può mancare" e "conosco già". Le due esclusioni chiedono all'AI cose diverse (*già note* alza il livello, *da evitare* non dice nulla sul livello) e non andavano unificate: è lo scenario che l'opzione «due liste separate», scartata in #28 su raccomandazione dell'operatore, copriva. Aggiunta la lista **«Non le voglio»** con divieti mirati alle vie di rientro, step 4 riformulato per essere trovabile, e vincolo «mai in prima tappa» nella progressione. ✅ Deployato (SW v7) e pushato, collaudo verde. ⚠️ **Resta da osservare se il modello OBBEDISCA**: è verificato che le istruzioni arrivino, non che vengano rispettate — Stefano lo prova rifacendo il percorso. — Precedente #28 (Opus 4.8). **Giro di debug su Syllabus e Footnote**, su tre richieste di Stefano. Footnote: trovata la ROOT CAUSE del "non compare la voce per rigenerare una parte" — `cardNeedsRepair` e `repairCard` avevano **due liste di requisiti divergenti**, il bottone era cieco su 5 campi su 11; ora una sola tabella `CARD_SECTIONS`, più un **menu di rigenerazione a spunte** che può sostituire anche ciò che è già completo (prima impossibile). Syllabus: **risorse «già note»** (escluse E date per acquisite, così il percorso riparte da lì) e **progressione a tappe motivate** (chiamata AI separata dopo la selezione, con riaggancio dei titoli a prova di errore dell'AI; i percorsi vecchi hanno un pulsante per ordinarsi senza rigenerarsi). Chiuse due trappole trovate strada facendo: `raw_data` ricostruito a mano perdeva i campi nuovi a ogni salvataggio, e le descrizioni sparivano nella vista a tappe. ✅ **Footnote (SW v29) e Syllabus (SW v6) DEPLOYATI e verificati live, e main PUSHATO** (`1e2577d..8aee6fe`); `collauda.cjs` tutto verde coi minimi nuovi. ⚠️ **ListenS non ancora guardato** (era nel giro): resta lì, col suo miglioramento generale da valutare. 🔍 **Tre bug latenti trovati e NON toccati** (fallimenti silenziosi mascherati da fallback, nessuno visibile oggi): `sl_resources` è una tabella morta con 0 righe mentre i dati vivono in `raw_data`, `updateCurriculum` riceve chiavi che non sono colonne, e `Syllabus/supabase-migration.sql` non descrive più lo schema reale — dettaglio e decisioni in coda al diario. — Precedente #27 (Opus 4.8). **▶ DnDMaster è CHIUSO.** In #27: trovata la ROOT CAUSE del "non vedo le modifiche" — **non era la cache**, il tab 🤝 Tavolo era assente dalla **bottom nav mobile** (sotto i 768px `.header-tabs` è `display:none`), quindi irraggiungibile su telefono pur essendo nel bundle; fix di 1 riga, deployato e **confermato da Stefano**. Fatto anche l'**auto-popolamento del Combat Tracker dai vitali** (snapshot all'avvio + ↻ per-PG, niente Realtime) e chiusa la coda dei **dadi vita del riposo lungo** (estratto `hitDiceAfterLongRest` + 4 test; la formula era già corretta). 186 test verdi. ⚠️ Deploy+push del tracker **da autorizzare**. — Precedente #26 (Opus 4.8): **nessun blocco obbligatorio aperto** sulle schede condivise DnD — **blocchi 3b + 3c + A + rifiniture FATTI, deployati e pushati** (tab 🤝 Tavolo: giocatore cura la scheda, master assegna + diff/accept + vitali live via Realtime **verificato**; blocco A = condizioni + dadi vita come campi persistenti; RPC `join_campaign` fixata). **Stefano valuta di CHIUDERE DnD domani e passare ad altro progetto** (candidato naturale se si continua: auto-popolamento del Combat Tracker dai vitali live). TO-DO futuri (non urgenti): campagna-scopare il roster del master, feedback master→giocatore (v2), auto-popolamento combat tracker dai vitali. **Non verificato a runtime**: recupero dadi vita del riposo lungo (ispezione). ✅ **3b+3c deployati e live** (Netlify, verificato: bundle contiene `Tavolo`/`join_campaign`) e **pushati** su GitHub; portano anche il blocco 1 ora live. Follow-up `backup.js` **chiuso** (3 tabelle aggiunte, cp-backup ridistribuito). Dettaglio in `DnDMaster/CLAUDE.md`, sezione «Schede condivise coi giocatori». — Precedente #25 (Opus 4.8, handover da Fable 5): Blocchi 1 (schede per-PG, deployate e live), 2 (tabelle `campaigns`/`dnd_shared_chars`) e 3a (`campaign_members` + RPC `join_campaign` + helper RLS) FATTI su pchld. ⚠️ follow-up: aggiungere le 3 tabelle a `Backup/api/backup.js` + redeploy cp-backup prima dei dati veri. — **Blocco 1 schede condivise DnD deployato**: schede personaggio ora una-riga-per-PG (`char:<id>` + indice `dnd_char_index_v1`), motore di sync esteso (char-key dinamiche, `markDeleted`/tombstone), migrazione non distruttiva (blob `dnd5e-master-v1` intatto = rollback) — su Netlify, **verificato live da Stefano**. Prima: design schede condivise esteso al **modello a campagna** (multi-master, join-code) e verifica primo backup Ledger 19/07 (✅ `projects.bogav`, 70 transazioni). Stato: piano-di-lavoro.md; diario in coda. — Precedente (#24). **Giro debug+features su segnalazioni di Stefano**: DnD (slot importate FIXATI, chip CD/attacco, ricerca incantesimi in italiano via dizionario 138 voci, design schede-giocatori ratificato), ListenS (pulsanti auto ⏮⏭, limite Android Auto chiarito), Digest (icona scura). Tutto deployato e verificato. Stato: piano-di-lavoro.md; diario in coda. In sospeso: ~~verifica primo backup Ledger (mattina del 19/07)~~ → ✅ **verificato 2026-07-19** (Opus): `backup-2026-07-19.json` (cron 05:07 UTC) contiene `projects.bogav` con 11 tabelle Ledger, `transactions` 70 righe, zero tabelle in errore; il 18/07 non ha ancora `bogav` — il 19 è il primo, come da RIPRISTINO.md.
...
**Stato attuale:** âœ… Completo con Supabase. **2026-07-20 (#28):** corretto il bottone «Completa scheda», che era cieco su 5 sezioni su 11 (`cardNeedsRepair` e `repairCard` avevano due liste divergenti — ora una sola tabella `CARD_SECTIONS`), e aggiunto il **menu «↺ Rigenera ▾»** con scelta a spunte delle sezioni, che a differenza della riparazione sostituisce anche ciò che è già completo. SW v29 — ✅ **deployato e verificato live 2026-07-20** (`curl` sw.js = v29; `CARD_SECTIONS` e il menu presenti nel file servito).
...
**Stato attuale:** ✅ Live. **2026-07-24 (#30):** corrette le statistiche per anno/mese —
`read_year`/`read_month` (colonne text) venivano caricate come stringhe e i confronti numerici
in `StatsView`/`StatsBar` erano sempre falsi (anno corrente/mesi vuoti); coercite a `Number` al
load (`App.jsx:1696`). Deployato Vercel, **SW `bookshelf-v3`**, verificato live.
...
**Tech stack:** single-file `index.html` (pattern Marginalia/Footnote: React inline + Babel standalone @7), PWA con `sw.js` network-first (`cru-v9`).

**Cartella:** `Claude/Progetti/Commonplace/Cru/` — **⚠️ ha un suo `CLAUDE.md` dedicato, leggilo prima di lavorarci** (specifiche uniche: progetto Supabase su un SECONDO account, repo git proprio, tranello deploy git-email).

**Stato attuale:** ✅ Considerata **buona/completa** (2026-07-22). Fase: condivisione con amici + troubleshooting.
...
- CRUD vino: cantina, nome, denominazione, tipologia, regione, annata, vitigni, formato, prezzo, gradazione, voto (Vivino 1–5 decimali), commento, **posizione in cantina**.
- Autocomplete denominazioni (dataset curato `denominazioni.js`, **251 voci**, Italia estesa + estero: regione, tipologia, vitigni, potenziale d'invecchiamento).
- Scheda di degustazione AIS completa (visivo/olfattivo/gustativo + descrittori + punteggio /100) in `tasting` jsonb; voto rapido richiesto al momento del "bevuto".
- Finestra di bevuta stimata (euristica, sovrascrivibile) + filtro a chip colorati.
- **Scan barcode con fotocamera** (BarcodeDetector) + lookup a cascata Open Food Facts → Open Products Facts → UPCitemdb (copertura vini parziale, limite DB gratuiti).
- Viste **Cantina / Wishlist / Archivio**; wishlist con "sposta in cantina"; export **CSV/JSON**.
- Statistiche + icone PWA PNG.
- **Multi-utente**: registrazione in-app (login/registrati/reset), ogni utente vede solo la sua cantina (RLS).

**Deploy:** Vercel — progetto `cru` (`olengards-projects/cru`), dominio `cru.commonplaceapp.org` ✅. Nessuna env var (anon key nel file). Collaudo locale: `cru-static` in launch.json (porta 5188). **⚠️ TRANELLO**: i deploy git-backed sono bloccati da Vercel se l'email del commit HEAD non è di un account GitHub → nel repo Cru `git config user.email olengard@gmail.com` (dettaglio in `Cru/CLAUDE.md`).

**Supabase:** ⚠️ **progetto DEDICATO `fqdccttzuaopoaojshvw` su un SECONDO account di Stefano** (NON `pchld`, per isolare gli amici dalla suite personale). Tabelle `cru_wines`, `cru_movements`, RLS `user_id = auth.uid()`. **Il MCP Supabase è collegato al primo account: NON può gestire il progetto Cru** — schema/settings dal dashboard del secondo account. Restano orfane su `pchld` le vecchie `cru_wines`/`cru_movements` (3 vini di test, droppabili).

**Git:** repo **dedicato e privato** `github.com/Olengard/cru` (NON il repo esterno/syllabus, per azzerare il rischio force-push). Unica app single-file della suite versionata in git.
...
**Stato attuale:** Funzionante — revisione completa 2026-06-09 (Sessione #18). ⚠️ Da fare: impostare nuova `ANTHROPIC_API_KEY` su Vercel (progetto Syllabus) + redeploy. **2026-07-20 (#28):** aggiunte le **risorse «già note»** (step 4 del wizard: escluse dal percorso E date per acquisite, così l'AI riparte da lì invece di accorciare) e la **progressione a tappe motivate** (chiamata AI separata dopo la selezione delle risorse; la call A non è stata toccata; i percorsi esistenti hanno «↓ Genera la progressione» che li ordina senza rigenerarli). **2026-07-21 (#28b):** aggiunta la **terza lista «Non le voglio»** (`avoidList`) — le due esclusioni sono ora distinte, perché «già note» alza il livello di partenza mentre «da evitare» non deve dedurne nulla; step 4 riformulato in «Cosa includere, cosa lasciare fuori» per renderlo trovabile; la progressione non mette mai in prima tappa ciò che l'utente evita o conosce già. SW v7 ✅ deployato e verificato live. I campi nuovi vivono in `raw_data`, **nessuna migration**. SW v6 — ✅ **deployato e verificato live 2026-07-20**: bundle servito `index-Cd5XT5BA.js` = build locale, marker presenti (`knownAlready`, `TAPPA`, «Genera la progressione»). ✅ Ricontrollato **dopo il push** che l'integrazione Git di Vercel sia ancora scollegata (l'incidente del 18/07): bundle invariato, nessun redeploy stantio. 🔍 Vedi i tre bug latenti in coda al diario (#28): `sl_resources` è morta (0 righe, dati in `raw_data`), `updateCurriculum` riceve non-colonne, e `supabase-migration.sql` **non descrive più lo schema reale** — non usarlo per ricreare il database.
...
## 2026-07-20 — Sessione #28 (Opus 4.8)

**Giro di debug su Syllabus e Footnote** (punto 3 del piano). Il giro era previsto anche su
ListenS, ma Stefano ha portato tre richieste concrete su Syllabus e Footnote e quelle hanno
avuto la precedenza sull'ispezione sistematica. **ListenS resta da guardare**, insieme alla
valutazione del suo miglioramento generale.

Baseline verificata all'apertura: `collauda.cjs` tutto verde, e locale = live per Footnote
(v28) e ListenS (v16). Il piano di lavoro dava ListenS a v13: era fermo a #24, l'app ha avuto
tre deploy dopo. `data-model.md` è del **23 marzo**, non della sessione Subs: falso allarme.

- ✅ **Footnote: root cause del "non sempre compare la voce per rigenerare una parte"** —
  `cardNeedsRepair()` (che decide se mostrare il bottone «🩹 Completa scheda») e `repairCard()`
  (che ripara) tenevano **due liste di requisiti scritte a mano, divergenti**. Il controllo
  ignorava `context`, `keyChapters`, `keyConcepts`, `keyPassages` e `glossary` — 5 campi su 11 —
  che invece la riparazione sapeva sistemare. Una scheda a cui mancava solo il glossario
  risultava "sana": bottone nascosto, unica via rigenerare tutto. Ora le liste sono **una sola
  tabella `CARD_SECTIONS`**, da cui derivano controllo, riparazione ed etichette del menu:
  tenerle separate è ciò che le ha fatte divergere.
- ✅ **Footnote: menu di rigenerazione selettiva** — «↺ Rigenera ▾» → tutta la scheda, oppure
  scelta a spunte per sezione (le incomplete sono marcate). Novità semantica: la scelta
  esplicita **sostituisce anche ciò che è già completo** (`force`), cosa che prima non si
  poteva chiedere in alcun modo — la riparazione per costruzione riempie solo i buchi.
  Le sezioni dello stesso gruppo nascono da una sola chiamata AI: sceglierne una non fa
  risparmiare token, ma **solo quella viene scritta**. SW v28 → v29.
- ✅ **Syllabus: risorse «già note»** — elenco nuovo nello step 4 del wizard, speculare ai punti
  fermi. Non è una semplice esclusione: le opere indicate vengono **date per acquisite**, e
  l'AI sceglie testi che presuppongono quella base o la contestano. Propagate alla chat di
  affinamento, che non può più riproporle.
- ✅ **Syllabus: progressione a tappe** — 3-5 tappe con motivazione ("perché questa tappa ora,
  cosa lascia alla prossima"). **Chiamata AI separata DOPO la selezione delle risorse**: la
  call A, tarata negli anni contro i troncamenti, non è stata toccata; se la progressione
  fallisce il percorso è quello di prima, solo senza tappe. I titoli tornati dall'AI vengono
  riagganciati alle risorse vere con confronto normalizzato; **le non riagganciate finiscono in
  coda invece di sparire** e le tappe vuote vengono eliminate rimappando gli indici (collaudato
  su tre casi con risposta AI simulata). Vista, PDF e chat sono a tappe. I percorsi vecchi
  ricadono sul raggruppamento per fase e hanno «↓ Genera la progressione», che **riordina le
  risorse esistenti senza rigenerarle**. SW v5 → v6.
- ✅ **Syllabus: due trappole chiuse strada facendo** — (a) ogni handler ricostruiva `raw_data`
  elencando i campi a mano, quindi ogni salvataggio **cancellava in silenzio ciò che non era
  nell'elenco**: tappe e già-note sarebbero morte alla prima spunta su una risorsa. Ora
  `buildRaw()`, un punto solo. (b) `ResourceCard` leggeva solo `note`, che esiste solo dopo
  `groupResources` (rinomina `description`→`note`): nella vista a tappe **le descrizioni
  sparivano**. Trovato solo perché la vista è stata guardata renderizzata, non dedotta.

### 🔍 Ritrovamenti del giro di debug — NON toccati, da decidere

Tre bug latenti della stessa famiglia: **fallimenti silenziosi mascherati da un fallback**.
Nessuno di questi ha effetti visibili oggi, nessuno perde dati. Non li ho corretti perché
fuori dall'ambito chiesto; sono tutti in `Syllabus/app`.

1. **`sl_resources` è una tabella morta** — 15 percorsi, 185 risorse: tutte in
   `sl_curricula.raw_data`, e `sl_resources` ha **0 righe** (verificato via SQL). L'insert in
   `saveCurriculum` non può riuscire: passa `phase` (colonna **inesistente** nello schema live)
   e **omette `section`** (`NOT NULL` senza default). L'errore è solo `console.error`, mai
   sollevato. L'app è immune perché legge e scrive tutto da `raw_data`. Da decidere: allineare
   l'insert, oppure **eliminare tabella e codice** e dichiarare `raw_data` la fonte di verità
   (è già di fatto così).
2. **`updateCurriculum` riceve chiavi che non sono colonne** — `onUpdate` → `handleCurriculumUpdate`
   rigira l'intero patch (che contiene `resources`, e ora `stages`) a `.update()` su
   `sl_curricula`. **Dedotto, non osservato**: PostgREST dovrebbe rifiutare con "column ... does
   not exist", e anche qui l'errore è solo loggato. I dati si salvano lo stesso perché
   `Curriculum.jsx` chiama `updateCurriculum` **anche direttamente** con le sole colonne valide.
   Conferma in 5 secondi: aprire un percorso, spuntare una risorsa, guardare la console.
3. **`Syllabus/supabase-migration.sql` è superato dai fatti** — descrive uno schema che non
   esiste più (mancano `topic`, `focus_areas`, `progress_pct`, `status`, `raw_data`…, e
   `sl_resources` ha colonne che il file non ha). Documento storico: **non usarlo per ricreare
   lo schema**.

✅ **Deploy e push FATTI** (autorizzati da Stefano a fine sessione):
- **Footnote** → Vercel, SW v28→v29. Verifica live: `sw.js` = v29, `CARD_SECTIONS` e
  «Rigenera solo alcune parti» presenti nel file servito, fix stacking (`zIndex:30`) incluso.
- **Syllabus** → Vercel da `Syllabus/app/`, SW v5→v6. Verifica live: bundle servito
  `index-Cd5XT5BA.js` **identico al build locale**, marker tutti presenti.
- **`SW_MIN` alzato** in `Suite/collauda.cjs` (footnote 29, syllabus 6) **solo dopo** il deploy,
  per non tenere rosso il collaudo nel frattempo → `collauda.cjs` **tutto verde**.
- **Push** del solo repo esterno: `1e2577d..8aee6fe`, main allineato a origin/main.
  `Syllabus/app` NON pushato (regola invariata: storia incompatibile col remote).
- ✅ Ricontrollato **dopo** il push che Vercel non abbia rideployato un Syllabus stantio
  (incidente del 18/07, integrazione Git): bundle invariato, integrazione ancora scollegata.

⚠️ **Restano aperti:**
- **ListenS**: giro di debug e valutazione del miglioramento generale — era nel punto 3 del
  piano, non è stato toccato.
- **I tre bug latenti di Syllabus** qui sopra: Stefano ha deciso di rimandarli all'ulteriore
  debug di Footnote e Syllabus, già concordato come prossimo lavoro.
- **Sospensione Render `digest-blqp` (~26/07)** — azione di Stefano sul dashboard.
- **Nessun prompt è mai stato provato contro il modello vero**: in sessione tutte le risposte
  AI erano simulate. La meccanica è verificata, la QUALITÀ delle tappe e l'effetto delle
  «già note» sul livello del percorso si vedono solo generando un percorso reale — è la
  prima cosa da provare ora che è live.
...
## 2026-07-21 — Sessione #28b (Opus 4.8) — coda di #28

**Segnalazione di Stefano dopo il deploy**: "ho indicato che non voglio McCarthy tra i
consigli e il primo libro consigliato è di McCarthy — sembra che l'exclude non funzioni".

- 🔍 **Il filtro non era rotto: non è mai entrato in funzione.** Verificato sui dati prima
  di qualunque ipotesi: il percorso (`a91d5af3…`, creato 20/07 22:05) ha `known_already: []`
  — il campo era vuoto. L'esclusione era stata scritta nell'**argomento**, in prosa: *"Oltre
  McCarthy: alternative per chi è interessato ai temi ma ne odia lo stile"*. L'AI l'ha letta
  come un tema **su** McCarthy e ha incluso *Blood Meridian* motivandolo così: «rileggere
  criticamente l'opera-fonte è indispensabile […] per usarlo come pietra di paragone».
  Compariva **per primo** perché la progressione (nuova, di #28) gli aveva dato la **tappa 0**.
- ✅ **ROOT CAUSE: errore di progettazione, non di codice.** Lo step 4 chiedeva "cosa non può
  mancare?" e "cosa conosci già?": **"non lo voglio" non era esprimibile**. Le due esclusioni
  chiedono all'AI cose diverse — *già note* **alza** il livello di partenza, *da evitare* non
  deve dire **nulla** sul livello — e comprimerle in una lista sola lasciava fuori metà dei
  casi. È esattamente lo scenario che l'opzione «due liste separate», **scartata in fase di
  design in #28 su raccomandazione dell'operatore**, copriva. Lezione: quando due esclusioni
  hanno motivazioni diverse, producono prompt diversi e non vanno unificate per semplicità.
- ✅ **Terza lista `avoidList`** con istruzioni opposte a «già note»: nessuna deduzione sul
  livello, e divieto **esplicito** di rientrare come "opera-fonte da rileggere criticamente"
  o come "termine di paragone" — le due vie da cui McCarthy era rientrato. Coperto anche il
  caso in cui è l'ARGOMENTO a dire "oltre X" / "alternative a X": X va evitato lo stesso.
- ✅ **Step 4 reso trovabile**: da «Punti fermi e già viste» a **«Cosa includere, cosa lasciare
  fuori»**, con le due esclusioni sotto un'unica domanda e due sottotitoli che dicono quale
  scegliere; tre toni di chip distinti. Il difetto vero era di scopribilità: l'utente col
  bisogno preciso non aveva raggiunto la funzione.
- ✅ **Progressione**: ciò che l'utente evita o conosce già **non può stare in PRIMA tappa**.
  Aprire un percorso "oltre X" con X lo rende inutile anche quando la motivazione è sensata.
- ⚠️ **Falso allarme evitato, da ricordare**: durante il collaudo l'Enter sembrava non
  aggiungere le voci nei campi a chip. **Non è un bug**: un `KeyboardEvent` autentico funziona
  (e il pulsante ＋ pure). Era una limitazione dello strumento di test del browser, che non
  produce un keydown visibile a React. Non "correggere" quel codice: non è rotto.

✅ **Deployato e pushato** (autorizzato da Stefano): Syllabus SW v6→v7, bundle servito
`index-Dm8VIhY0.js` **identico al build locale**, marker verificati live (`avoidList`,
«Non le voglio», «opera-fonte da rileggere criticamente», «PRIMA tappa»). `SW_MIN` syllabus
→ 7, `collauda.cjs` **tutto verde**.

⚠️ **DA VERIFICARE — spetta a Stefano, ed è il punto che conta:** il prompt ora vieta le
esclusioni in modo esplicito e ridondante, ma **nessuno ha ancora osservato se il modello
obbedisca**. In tutta la sessione le risposte AI sono state simulate: è verificato che le
...
## 2026-07-21 — Sessione #28c (Opus 4.8)

Due segnalazioni di Stefano dopo aver usato Syllabus dal vivo.

- ✅ **Le esclusioni ora funzionano DAVVERO — scarto deterministico.** In #28b avevo aggiunto
  la lista «Non le voglio» e verificato che i divieti ARRIVASSERO al modello, lasciando aperto
  se venissero rispettati. Stefano: in più percorsi McCarthy rientrava lo stesso. Confermato
  che il prompt da solo non basta. Aggiunto `filterExcluded()` in `generate.js`: dopo la
  generazione confronta titolo e autore di ogni risorsa con `avoidList` (match normalizzato —
  minuscole, accenti e apostrofi neutralizzati, sottostringa bidirezionale) e scarta i
  corrispondenti. Applicato dopo la Call A (il conteggio per il top-up è già al netto) e dopo
  il top-up (che potrebbe reintrodurne), più una guardia nella chat. **Non dipende
  dall'obbedienza del modello** → i test simulati sono prova PIENA, non parziale. Decisioni:
  filtra solo `avoidList` NON `knownAlready` (un altro Beckett può essere il passo giusto);
  NON tocca le descrizioni (un'opera esclusa resta nominabile per situarne un'altra — il caso
  Bloom approvato da Stefano). Verificato: 6 casi unitari + integrazione (modello propone 2
  McCarthy → filtro toglie → top-up rimpiazza → 0 McCarthy, lunghezza piena). SW v8→v9.
- ✅ **PWA installabile con icona dedicata.** Due difetti: (1) il manifest dichiarava tutte le
  icone come `favicon.png`, un file da 646 byte (32px) spacciato per 192 E 512 — mislabeling
  che fa rifiutare l'install; (2) `icon-192.png`/`icon-512.png` presenti erano **l'icona di
  BookShelf** (una "B" con scritto "BookShelf"), finite qui per copia. Generate le icone
  dedicate di Syllabus (la "S" in Playfair oro su fondo scuro, cornice + "Syllabus" in DM Mono,
  gemella delle altre app) su canvas alle risoluzioni vere: 192, 512, maskable-512 full-bleed,
  apple-touch 180. Manifest ricucito (sizes reali, `scope:"/"`), `apple-touch-icon` → il PNG
  dedicato. Verificato live: manifest con le 3 icone nuove, ognuna PNG valido 200 image/png
  delle dimensioni dichiarate, display standalone. Le icone sono in `Syllabus/app/public/`.
  Nota tecnica: nessun rasterizzatore sul PC (né sharp/canvas/ImageMagick/rsvg) → generate
  renderizzandole su `<canvas>` nel browser di preview e POSTate a un ricevitore HTTP locale
  usa-e-getta, per non trascrivere base64 a mano. Metodo riutilizzabile per le altre app.

✅ **Deployato e pushato**: Syllabus SW v8→v9, bundle live, tutto verificato con curl.
`SW_MIN` syllabus → 9, `collauda.cjs` verde.

⚠️ **Da verificare (Stefano)**: reinstallare la PWA sul dispositivo (disinstallare prima quella
vecchia, poi reinstallare — l'icona vecchia può restare in cache dal sistema operativo). E
confermare che l'esclusione tenga anche su percorsi nuovi generati dal vivo (ora ha una difesa
deterministica, ma un giro reale in più non fa mai male).

---

## 2026-07-21 — Sessione #28d (Opus 4.8)

Stefano: "ho messo McCarthy nell'esclusione e i primi due libri sono di McCarthy — sbaglio io?"

- 🔍 **NON sbagliava lui: il campo perdeva il testo.** Verificato sui dati PRIMA di ipotizzare:
  TUTTI i percorsi di Stefano con McCarthy hanno `avoidList` **vuota** (query su sl_curricula).
  Il filtro deterministico di #28c era corretto ma non aveva nulla da filtrare. Due cause,
  entrambe di progettazione mia:
- ✅ **(1) TagField buttava via il testo non confermato.** Il testo si aggiungeva solo su Invio
  o clic sul ＋; scrivere "McCarthy" e cliccare "Continua" lo perdeva. **È lo stesso
  comportamento che in #28c avevo visto in collaudo e archiviato come "artefatto del test": era
  il bug vero, l'ho liquidato due volte.** Ora `onBlur` committa il pendente (il ＋ usa
  onMouseDown+preventDefault per non rubare il blur → niente doppio inserimento), più un avviso
  quando c'è testo non aggiunto.
- ✅ **(2) Stefano scrive l'esclusione nell'ARGOMENTO, non nel campo — due volte su due.**
  ("Oltre McCarthy: …stile detestabile"). `estraiEsclusioni()` rileva "oltre/contro/senza/
  tranne/eccetto X" e propone X come chip da toccare (opt-in: interpretazione sbagliata innocua
  se non la tocchi), con stoplist per non estrarre periodi/luoghi ("Oltre il Novecento" → nulla).
- ⚠️ **Lezione di metodo che è costata tempo:** gli eventi sintetici (`dispatchEvent` di
  blur/focus) **non attraversano la delega `focusout` di React** — tre test "falliti" prima di
  capire che era lo strumento, non il codice. Per validare focus/blur nel wizard serve il
  **mouse reale** (computer tool), non `dispatchEvent`. La verifica finale con mouse+tastiera
  reali: argomento "Oltre McCarthy" → suggerimento "+ McCarthy" → tocco → chip; "Faulkner"
  digitato e committato cliccando altrove senza Invio; payload `avoidList=["McCarthy","Faulkner"]`.

✅ **Deployato e pushato**: SW v9→v10, bundle live = build locale, marker verificati. collauda verde.

⚠️ **Test per Stefano** (ora la difesa è a tre strati: suggerimento dall'argomento + commit del
campo + filtro deterministico): rifare "oltre McCarthy" e, allo step 4, o toccare il
suggerimento "+ McCarthy" o scriverlo nel campo (ora tiene anche senza Invio). McCarthy deve
sparire. Se ancora rientra, guardare `avoidList` del percorso salvato (query sl_curricula):
se è popolata e McCarthy resta, è il filtro; se è vuota, è ancora la UI.

---

## 2026-07-24 — Sessione #30 (Opus 4.8)

Due lavori su segnalazione di Stefano: DnD Master (reputazione) e BookShelf (statistiche).

- ✅ **DnD Master — «Aggiorna reputazione del giocatore» (alias che non arrivava).** Stefano:
  "imposto che il giocatore veda «Famiglia» invece di «Clero», ma continua a vedere «Clero»".
  🔍 **ROOT CAUSE verificata sui dati reali** (query pchld, non per ipotesi): l'alias/nascondi
  del prestigio raggiunge la scheda del giocatore in **un solo punto** — `seedSharedChar` →
  `sanificaPerGiocatore` → `publicPrestige` — che gira **solo all'assegnazione**. Impostare
  l'alias *dopo* non arriva mai: in v1 non c'è sync amministrativa master→giocatore. Prova:
  la riga `dnd_shared_chars` di Teofilo aveva ancora `{id:4, name:"Clero"}` (nome vero, zero
  alias), mentre la copia di roster del master era ormai divergente. Le due copie sono
  congelate al seed. **Fix mirato (NON un re-seed**, che ri-spingerebbe l'intera copia master
  cancellando la scheda compilata dal giocatore e i suoi vitali**):**
  - `sharedChar.refreshPublicPrestige(master, shared)` (puro): forma pubblica aggiornata
    (alias applicati, nascoste tolte, rinomini) **preservando i valori del giocatore per id**
    (nomi/alias/visibilità = autorità master; i valori li possiede il giocatore, canale
    propose→accept). Voci nuove del master entrano col loro valore.
  - `sharedSync.refreshSharedPrestige()`: legge la riga, riscrive **solo** `char.prestige`,
    resto intatto. RLS `dsc_update_own` consente al master l'UPDATE delle righe delle sue
    campagne (**verificata su pg_policy**, non dedotta).
  - `SharedTablePage`: bottone **«🏛 Aggiorna reputazione»** per riga assegnata (visibile solo
    se il PG è ancora nel roster del master).
  - **238 test verdi** (6 nuovi: pure + trasporto contro fake-client — preservazione valori,
    alias, drop nascoste, voci nuove, riga inesistente). Build verde.
  - ✅ **Deployato su Netlify e verificato live** (`dnd.commonplaceapp.org`, bundle
    `index-C4kw_MT2.js` contiene «Aggiorna reputazione»). SW vite-pwa autoUpdate (no bump).
  - ⚠️ **Verifica finale a carico di Stefano** (richiede master + giocatore loggati, due
    account): imposta l'alias, clicca «🏛 Aggiorna reputazione» sulla riga del giocatore, e il
    giocatore ricarica la scheda condivisa → deve vedere «Famiglia». Nel test il flusso a due
    account non era esercitabile da qui. **Nota pratica:** nel tuo test la copia di roster non
    ha più «Clero»/«Famiglia» (l'hai modificata durante le prove) — per riprovare, rimetti
    l'alias su una voce e poi premi il bottone.

- ✅ **BookShelf — statistiche per anno/mese vuote per l'anno corrente.** 🔍 ROOT CAUSE:
  `read_year`/`read_month` sono colonne **text** su Supabase; caricate come stringhe
  (`App.jsx:1696`), i confronti numerici in `StatsView`/`StatsBar` (`=== scopeYear` da
  `getFullYear`, `=== i+1` da `getMonth`) erano **sempre falsi** → l'anno selezionato di
  default e i mesi apparivano vuoti. La barra "libri per anno" reggeva perché confrontava
  stringa-con-stringa. **Fix**: coercizione a `Number` al load (vuoto = `""`), coerente coi
  libri creati dal form (`parseInt`). Tracciati tutti i punti a valle (chip anno, `filterYear`,
  `statsYear`, `byYear`): ora numero-vs-numero. Build verde.
  - ✅ **Deployato su Vercel e verificato live** (`bookshelf.commonplaceapp.org`, SW
    **`bookshelf-v3`**, bundle `index-DZp84SOy.js` con marker `read_year==null`).
  - ⚠️ **Prova visiva finale a Stefano** (l'app è dietro login, non esercitabile da qui): il
    KPI **«LIBRI LETTI NEL 2026»** deve mostrare il conteggio e i mesi/anno 2026 popolarsi.
    Sul dispositivo può servire un paio di reload (SW) perché la PWA prenda la v3.

- `Suite/collauda.cjs`: `SW_MIN.bookshelf` 2→3. **Collaudo suite tutto verde** dopo i deploy.
- ⚠️ **Azioni tue ancora aperte (da sessioni precedenti):** sospensione formale Render
  `digest-blqp` (~26/07), ricarica vini sul progetto Cru `fqdcc…`, cancella l'utente di
  collaudo Cru. **Ancora in coda come lavoro:** giro di debug ListenS + ulteriore debug
  Footnote/Syllabus (coi tre bug latenti di Syllabus).
