...
|---|---|---|
| `pchldmiavycxzpkzochn` ("NoteS") | **Progetto principale della suite** | `bs_*` (BookShelf), `fn_books` (Footnote), `ls_*` (ListenS), `sl_*` (Syllabus), `dnd_saves` (DnDMaster), `cp_items`/`cp_log`/`cp_quotes` (layer unificato + Marginalia), `dg_*` (Digest), tabelle NoteS, e da 2026-07 **Platea**: `videos`/`channels`/`carousels`/`carousel_videos`/`saved_videos`/`watch_progress`/`pl_*` + vista `continue_watching` + RPC `search_videos` (⚠️ anon-write per design su 7 di queste: Platea non ha login) + Edge Function `sync-videos` + job `pg_cron` |
| ~~`llvqoiyvzloloobjiloe`~~ | **DISMESSO** (Fase 7, 2026-07-17) | Era Platea/cp legacy/Dashboard su un altro account. Migrato TUTTO su pchld (tabelle video `videos`/`channels`/`carousels`/`carousel_videos`/`saved_videos`/`watch_progress`/`pl_*` + vista `continue_watching` + RPC `search_videos` + enum). Delta finali: zero (verificato). NON riattivarlo né referenziarlo in codice nuovo; storia in `Platea/piano-migrazione-pchld.md` |
